...
% \todo{State the source utility and the anonymisation rule agreed with the supervisor.}
...
$7.2\times10^{8}$~ppm), which motivates outlier clipping. Figure~\ref{fig:gasdist} shows the
per-gas distributions on a logarithmic scale.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{eda_gas_distributions}
  \caption{Per-gas concentration distributions ($\log(1+x)$). The heavy right-skew motivates
  the log transform; acetylene is zero in most samples.}
  \label{fig:gasdist}
\end{figure}
