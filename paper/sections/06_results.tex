...
$0.12$ for raw proportions (Fig.~\ref{fig:sdcae}). The improvement is therefore due
...
visibly (Fig.~\ref{fig:map}). Because Duval is a deterministic rule rather than an
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{representation_ablation}
  \caption{Fault-type agreement with Duval across representations (mean$\pm$std, five seeds).
  The gain comes from the CLR log-ratio geometry; a linear PCA projection beats the SD-CAE
  autoencoder, reported as a negative ablation.}
  \label{fig:sdcae}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\columnwidth]{clr_pca_map}
  \caption{Learned label-free diagnostic map: PCA-2 of the standardised CLR coloured by Duval
  class. Discharge (D1/D2), partial discharge, and thermal (T1--T3) regions separate.}
  \label{fig:map}
\end{figure}
...
confounded label (Fig.~\ref{fig:confound}), so the comparison must be read against that floor
...
$[-0.10,+0.06]$, $p=0.58$) and far above the conventional baselines (Fig.~\ref{fig:confound}).
...
chance ($0.50$, Fig.~\ref{fig:holdout}). Because the first-half sample count is known
...
Fig.~\ref{fig:chem}); the result is robust across nine onset-threshold and horizon variants
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.96\columnwidth]{label_confound_floor}
  \caption{The sample count (confound) out-predicts the physics index and every conventional
  severity baseline against the field-note label.}
  \label{fig:confound}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\columnwidth]{label_confound_holdout}
  \caption{Strict temporal hold-out: the score's forward validity vanishes once the sample-count
  confound is controlled, while the count alone forward-predicts.}
  \label{fig:holdout}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\columnwidth]{chemistry_target_arcing_onset}
  \caption{Confound-free chemistry target (arcing onset): gas chemistry predicts, the sampling
  confound does not.}
  \label{fig:chem}
\end{figure}
